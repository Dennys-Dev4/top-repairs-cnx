...
export default async function HomePage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <>
      ...
      {/* Hero Section */}
      <section className="hero-gradient relative min-h-[65vh] flex items-center px-4 overflow-hidden pt-12 pb-8">
        {/* Gradient orbs */}
        <div className="orb orb-orange w-[600px] h-[600px] top-[-10%] right-[-15%]" />
        <div className="orb orb-blue w-[400px] h-[400px] bottom-[5%] left-[-10%]" />
        <div className="orb orb-purple w-[300px] h-[300px] top-[30%] left-[15%] opacity-30" />
        ...
        <div className="max-w-7xl mx-auto text-center w-full relative z-10">
          ...
          <AnimateOnScroll>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-5 mb-6">
              <Link href={`/${lang}/booking`} className="btn-primary text-[18px] !px-12 !py-5">
                {t(lang, 'hero_cta')}
              </Link>
              <Link href={`/${lang}/contact`} className="btn-secondary text-[18px] !px-12 !py-5">
                {t(lang, 'get_estimate')}
              </Link>
            </div>
          </AnimateOnScroll>
          <AnimateOnScroll>
            <div className="flex items-center justify-center gap-4 text-text-muted text-[14px]">
          ...
          <AnimateOnScroll>
            <div className="grid grid-cols-3 gap-6 max-w-2xl mx-auto mt-14 pt-10 border-t border-border/40">
              {stats.map((stat) => (
                <div key={stat.key} className="text-center">
                  <span className="text-[28px] block mb-2">{stat.icon}</span>
                  <span className="text-[15px] font-semibold text-text-primary tracking-tight">{t(lang, stat.key)}</span>
                </div>
              ))}
            </div>
          </AnimateOnScroll>
          ...
          <AnimateOnScroll>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="card text-center !p-10">
                <span className="text-[48px] mb-5 block">{'\uD83D\uDCCD'}</span>
